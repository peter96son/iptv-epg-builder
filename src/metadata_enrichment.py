...
def _clean_generated_imdb_suffix(text: str) -> str:
    """Remove only the compact technical IMDb suffix generated by older builder versions."""
    value = (text or "").strip()
    value = re.sub(
...
def _add_metadata(
    ...
    for genre in genres:
        if normalize_name(genre) in existing_categories: continue
        c = ET.Element("category", {"lang": "ru"}); c.text = genre; programme.append(c); existing_categories.add(normalize_name(genre)); changed = True
    desc = programme.find("desc")
    existing = (desc.text or "").strip() if desc is not None else ""
    base = _clean_generated_imdb_suffix(existing)
    body = base or overview
    lines = []
    if genres: lines.append("Жанр: " + ", ".join(genres) + ".")
    if body: lines.append(body)
    if rating:
        rating_line = f"IMDb {rating}/10"
        if imdb_votes:
            pretty_votes = f"{int(imdb_votes):,}".replace(",", " "); rating_line += f" · {pretty_votes} голосов"
        lines.append(rating_line)
    if lines:
        rendered = "\n".join(lines)
        if desc is None: desc = ET.Element("desc", {"lang": "ru"}); programme.append(desc)
        if (desc.text or "").strip() != rendered: desc.text = rendered; changed = True
    return changed
